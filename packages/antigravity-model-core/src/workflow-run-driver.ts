/**
 * Antigravity Workflow Runtime — 运行驱动器 (WorkflowRunDriver)
 *
 ...
 * - WorkflowRunDriver 负责实际驱动: 排队 → 执行 → GAP_CHECK → 事件 → 检查点
 ...
 */
import type { WorkflowState, WorkflowEventEnvelope, RunGraph, CheckpointDTO } from '@anthropic/antigravity-shared'
import {
  createEventId,
  createISODateTime,
  createCheckpointId,
...
import type { EventStore } from '@anthropic/antigravity-core'
import type { CheckpointStore } from '@anthropic/antigravity-core'
...
export interface DrainOptions {
  ...
  /** 节点执行完成后的回调 */
  onNodeComplete?: (nodeId: string, result: NodeExecutionResult, lease: StepLeaseContext) => void | Promise<void>
  /** 节点执行失败后的回调 */
  onNodeFailed?: (nodeId: string, error: Error) => void | Promise<void>
  /** 节点被策略跳过后的回调 */
  onNodeSkipped?: (nodeId: string, detail: {
    sourceNodeId: string
    reason: string
    strategyId?: string
...
export interface WorkflowRunDriverDeps {
  ...
  issueStepLease?: (params: {
    runId: string
    nodeId: string
    graph: RunGraph
    state: WorkflowState
    attempt: number
  }) => Promise<StepLeaseContext> | StepLeaseContext
 ...
 *
 * Workflow 路由规则:
 * - PARALLEL → 仅在 adaptive policy 满足时跳过 DEBATE
 * - DEBATE → 强制进入 VERIFY，不允许跳过
 * - VERIFY → compliance=VIOLATION → 路由到 HITL
 */
function readModelConfidence(output: Record<string, unknown>, key: 'codex' | 'gemini'): number {
  const modelOutput = output[key]
  if (!modelOutput || typeof modelOutput !== 'object') {
    return 0
  }
  const confidence = (modelOutput as Record<string, unknown>).confidence
...
export function evaluateTransition(
  ...
  const skipNodes: TransitionDecision['skipNodes'] = []
  const forceQueue: string[] = []

  if (nodeId === 'PARALLEL') {
    const debateNode = state.capturedContext?.graph?.nodes.find(node => node.id === 'DEBATE')
    const skipPolicy = debateNode?.input?.skipPolicy as
      | { strategyId?: string; when?: string[] }
      | undefined
    const bothAvailable = output.bothAvailable === true
    const disagreement = typeof output.disagreementScore === 'number' ? output.disagreementScore : 1
    const minConfidence = Math.min(
      readModelConfidence(output, 'codex'),
      readModelConfidence(output, 'gemini'),
    )

    if (
      debateNode?.skippable === true &&
      skipPolicy &&
      bothAvailable &&
      disagreement <= 0.5 &&
      minConfidence >= 85
      ...
      skipNodes.push({
        ...
        reason: `Adaptive debate skip authorized after PARALLEL (disagreementScore=${disagreement}, minConfidence=${minConfidence}).`,
        strategyId: typeof skipPolicy.strategyId === 'string'
          ? skipPolicy.strategyId
          : 'adaptive.debate-express.v1',
        triggerCondition: Array.isArray(skipPolicy.when) && skipPolicy.when.length > 0
          ? `${skipPolicy.when.join(' && ')} [actual: disagreementScore=${disagreement}, minConfidence=${minConfidence}]`
          : `PARALLEL.bothAvailable === true && PARALLEL.disagreementScore <= 0.5 && PARALLEL.minConfidence >= 85 [actual: disagreementScore=${disagreement}, minConfidence=${minConfidence}]`,
      })
      forceQueue.push('VERIFY')
    }
  }

  if (nodeId === 'VERIFY') {
    const complianceCheck = (output.complianceCheck as string) ?? 'PASS'
 ...
 *
 * 不同于旧的初始化式应用服务入口，
 * WorkflowRunDriver.drain() 会实际执行所有节点到完成。
 */
export class WorkflowRunDriver {
  ...
  private readonly issueStepLease?: WorkflowRunDriverDeps['issueStepLease']
  private readonly validateNodeResult?: WorkflowRunDriverDeps['validateNodeResult']
  ...
  constructor(deps: WorkflowRunDriverDeps) {
    ...
    this.issueStepLease = deps.issueStepLease
    this.validateNodeResult = deps.validateNodeResult
  }
   ...
   */
  async drain(runId: string, opts: DrainOptions = {}): Promise<WorkflowState> {
    const maxTicks = opts.maxTicks ?? 20
    let tickCount = 0

    for (;;) {
      if (opts.signal?.aborted) {
        throw new Error('Workflow 运行被取消')
      }

      tickCount++
      if (tickCount > maxTicks) {
        throw new Error(`Workflow 运行超过最大 tick 数 (${maxTicks})，可能存在死循环`)
      }
      ...
      if (!result.progressMade) {
        ...
        throw new Error('Workflow 运行死锁: 无就绪节点且无运行中节点，但运行未终止')
      }
    }
  }
  ...
  private async appendWithRetry(
    runId: string,
    eventFactory: WorkflowEventEnvelope[] | ((baseVersion: number) => WorkflowEventEnvelope[]),
  ...
  private async executeNode(
    ...
    const lease = this.issueStepLease
      ? await this.issueStepLease({ runId, nodeId, graph, state, attempt: 0 })
      : {
          leaseId: createEventId(),
          runId,
          nodeId,
          attempt: 0,
          issuedAt: createISODateTime(),
          expiresAt: createISODateTime(),
    ...
    await this.safeEmitNodeStarted(runId, nodeId, executor.nodeId, lease)
    ...
    for (let attempt = 0; attempt <= executor.maxRetries; attempt++) {
      ...
      try {
        ...
        await this.safeEmitNodeHeartbeat(runId, nodeId, ctx.lease)
        result = await executor.execute(ctx)
        if (this.validateNodeResult) {
          result = await this.validateNodeResult({
            runId,
            nodeId,
            graph,
            state,
        ...
        // E. 完成前 Gate — 先走外部完成协议，只有成功后才允许真正 completed
        await opts.onNodeComplete?.(nodeId, result, lease)
    ...
    await this.safeEmitNodeFailed(runId, nodeId, lastError?.message ?? 'Unknown error', lease)
    ...
    if (transition.skipNodes.length > 0) {
      for (const skip of transition.skipNodes) {
        ...
        if (st.nodes[skip.nodeId]?.status === 'pending') {
          ...
          await opts.onNodeSkipped?.(skip.nodeId, {
            sourceNodeId,
            reason: skip.reason,
    ...
    if (transition.forceQueue.length > 0) {
      for (const queueNodeId of transition.forceQueue) {
        ...
        if (fqStatus === 'pending' || !fqStatus) {
          const queueEvent: WorkflowEventEnvelope = {
  ...
  private async safeEmitNodeStarted(
    runId: string,
    nodeId: string,
    model: string | undefined,
    lease: StepLeaseContext,
  ): Promise<void> {
    const evts = await this.eventStore.load({ runId })
    const st = projectStateFromEvents(runId, evts)
    const event: WorkflowEventEnvelope = {
      ...
      payload: {
        ...
        allowedModelPool: lease.allowedModelPool,
  ...
  private async safeEmitNodeHeartbeat(
    runId: string,
    nodeId: string,
    lease: StepLeaseContext,
  ): Promise<void> {
  ...
  private async safeEmitNodeFailed(runId: string, nodeId: string, error: string, lease?: StepLeaseContext): Promise<void> {
    const evts = await this.eventStore.load({ runId })
    const st = projectStateFromEvents(runId, evts)
    const event: WorkflowEventEnvelope = {
      eventId: createEventId(),
      runId,
      type: 'NODE_FAILED',
      payload: { nodeId, error, leaseId: lease?.leaseId },
      timestamp: createISODateTime(),
      version: st.version + 1,
    }
    await this.appendWithRetry(runId, [event], st.version)
  ...
  private async safeEmitNodeSkipped(runId: string, nodeId: string, reason: string, lease?: StepLeaseContext): Promise<void> {
    const evts = await this.eventStore.load({ runId })
    const st = projectStateFromEvents(runId, evts)
    const event: WorkflowEventEnvelope = {
      eventId: createEventId(),
      runId,
      type: 'NODE_SKIPPED',
      payload: { nodeId, reason, leaseId: lease?.leaseId },
      timestamp: createISODateTime(),
      version: st.version + 1,
    }
    await this.appendWithRetry(runId, [event], st.version)
   ...
   * - 任何节点 failed → 'failed'
   */
  private async emitRunCompleted(runId: string, state: WorkflowState): Promise<void> {
    // 避免重复发射
  ...
  /** 检查运行是否已终止 */
  private isTerminal(state: WorkflowState, graph: RunGraph): boolean {
    // 如果状态已标记为终态，直接返回
    if (state.status === 'completed' || state.status === 'failed' || state.status === 'cancelled') {
      return true
    }
