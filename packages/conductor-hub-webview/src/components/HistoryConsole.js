...
const HistoryConsole = ({ lang }) => {
    ...
    return (<div style={{ display: 'flex', height: '100%', gap: '0', borderRadius: theme_1.radius.md, overflow: 'hidden', boxShadow: theme_1.shadow.card, border: '1px solid var(--vscode-panel-border)' }}>
            {/* ── Sidebar ──────────────────────────────────────────────── */}
            <div style={{
            width: '300px',
            ...
            display: 'flex', flexDirection: 'column',
                ...
                <div style={{ padding: '10px', borderBottom: '1px solid var(--vscode-panel-border)' }}>
                    <input type="text" placeholder={T.search} value={search} onChange={e => setSearch(e.target.value)} style={{
            ...theme_1.s.input,
            fontSize: '12px',
            padding: '7px 10px',
        }}/>
                ...
                <div style={{ flex: 1, overflowY: 'auto' }}>
                    {filtered.map(record => (<div key={record.id} onClick={() => setSelectedId(record.id)} style={{
                padding: '12px 14px',
                cursor: 'pointer',
                ...
                borderLeft: `3px solid ${record.status === 'success' ? theme_1.colors.success : theme_1.colors.error}`,
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', color: 'var(--vscode-descriptionForeground)', marginBottom: '4px' }}>
                                ...
                                <span style={{ color: record.status === 'success' ? theme_1.colors.success : theme_1.colors.error, fontWeight: 500 }}>
                                    {record.duration}ms
                                </span>
                            </div>
                            ...
                            {(record.inputTokens || record.outputTokens) ? (<div style={{ fontSize: '11px', color: 'var(--vscode-textPreformat-foreground)', opacity: 0.8 }}>
            ...
            <div style={{ flex: 1, padding: '20px', overflowY: 'auto' }}>
                {selectedRecord ? (<div style={{ maxWidth: '800px' }}>
                        <h2 style={{ marginTop: 0, marginBottom: '12px', fontFamily: 'monospace', fontSize: '16px' }}>{selectedRecord.method}</h2>
                        <div style={{ marginBottom: '16px', display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                            <span style={{
                display: 'inline-block', padding: '2px 10px', borderRadius: theme_1.radius.pill,
                fontSize: '11px', fontWeight: 600,
                background: selectedRecord.status === 'success' ? theme_1.colors.success : theme_1.colors.error,
                color: '#fff',
            }}>
                                {selectedRecord.status.toUpperCase()}
                            </span>

                            {selectedRecord.model && selectedRecord.model !== 'unknown' && (<span style={{
                    display: 'inline-block', padding: '2px 10px', borderRadius: theme_1.radius.pill,
                ...
                color: 'var(--vscode-foreground)',
                            ...
                            {(selectedRecord.inputTokens || selectedRecord.outputTokens) ? (<span style={{
                    display: 'inline-block', padding: '2px 10px', borderRadius: theme_1.radius.pill,
                    fontSize: '11px', border: '1px solid var(--vscode-input-border)',
                    color: 'var(--vscode-foreground)',
                }}>
                ...
                padding: '14px', borderRadius: theme_1.radius.md,
                overflowX: 'auto', fontSize: '12px', whiteSpace: 'pre-wrap',
                fontFamily: 'var(--vscode-editor-font-family)',
                border: '1px solid var(--vscode-panel-border)',
                boxShadow: theme_1.shadow.card,
            }}>
...
//# sourceMappingURL=HistoryConsole.js.map