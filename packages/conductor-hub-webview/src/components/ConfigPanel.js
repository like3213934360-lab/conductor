...
const ModelCard = ({ model, apiKey, lang, initialTestResult, onEdit, onRemove, onToggle }) => {
    ...
    return (<div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '8px' }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px', flexWrap: 'wrap' }}>
                        ...
                        <span style={{ fontSize: '11px', color: theme_1.providerColors[def?.group || ''] || 'var(--vscode-descriptionForeground)', fontWeight: 500 }}>{model.label}</span>
                        {def?.pricing && (<span style={{ fontSize: '10px', color: 'var(--vscode-descriptionForeground)', background: 'var(--vscode-input-background)', border: '1px solid var(--vscode-input-border)', borderRadius: '3px', padding: '0 5px' }}>
                                {formatPrice(def.pricing.input, lang)} / {formatPrice(def.pricing.output, lang)} per M
                            </span>)}
                    </div>
                    {/* Description */}
                    {def && (<div style={{ fontSize: '11px', color: 'var(--vscode-descriptionForeground)', margin: '3px 0 7px' }}>
                            {def.note}
                        </div>)}
                    {/* Task badges */}
                    <div>
            ...
            cursor: testState === 'testing' ? 'default' : 'pointer',
                    ...
                    <button style={{ ...theme_1.s.btnSecondary, padding: '3px 10px', fontSize: '11px' }} onClick={() => onToggle(model.id, !model.enabled)}>
                        {model.enabled ? (lang === 'zh' ? '禁用' : 'Disable') : (lang === 'zh' ? '启用' : 'Enable')}
                    </button>
                    <button style={{ ...theme_1.s.btnSecondary, padding: '3px 10px', fontSize: '11px' }} onClick={() => onEdit(model, apiKey)}>
                        {lang === 'zh' ? '编辑' : 'Edit'}
                    </button>
                    {!confirmDelete ? (<button style={{ ...theme_1.s.btnSecondary, padding: '3px 10px', fontSize: '11px', color: 'var(--vscode-errorForeground)' }} onClick={() => setConfirmDelete(true)}>
                            {lang === 'zh' ? '删除' : 'Delete'}
                        </button>) : (<span style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
                            <span style={{ fontSize: '10px', color: 'var(--vscode-errorForeground)' }}>{lang === 'zh' ? '确认?' : 'Sure?'}</span>
                            <button style={{ ...theme_1.s.btnSecondary, padding: '2px 8px', fontSize: '11px', background: 'var(--vscode-errorForeground)', color: '#fff', border: 'none' }} onClick={() => { setConfirmDelete(false); onRemove(model.id); }}>
                                {lang === 'zh' ? '确认删除' : 'Yes'}
                            </button>
                            <button style={{ ...theme_1.s.btnSecondary, padding: '2px 8px', fontSize: '11px' }} onClick={() => setConfirmDelete(false)}>
                                {lang === 'zh' ? '取消' : 'No'}
                            </button>
                        </span>)}
                </div>
            ...
            <div style={{ marginTop: '7px', fontSize: '11px', color: 'var(--vscode-descriptionForeground)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '4px' }}>
                ...
                {testState !== 'idle' && (<span style={{ color: testColor, fontWeight: 500 }}>
...
const AddEditModal = ({ lang, existing, onSave, onClose }) => {
    ...
    const stepHeader = (n, title) => (<div style={{ marginBottom: '16px' }}>
            ...
            <div style={modal}>
                ...
                {step === 1 && (<>
                        ...
                        <div style={{ marginBottom: '14px' }}>
                            <label style={theme_1.s.label}>{lang === 'zh' ? '提供商' : 'Provider'}</label>
                            <select style={theme_1.s.select} value={selectedGroup} onChange={e => handleGroupChange(e.target.value)}>
                                {GROUPS.map(g => <option key={g} value={g}>{g}</option>)}
                            </select>
                        </div>

                        {!isCustomGroup && (<div style={{ marginBottom: '14px' }}>
                                <label style={theme_1.s.label}>{lang === 'zh' ? '型号' : 'Model'}</label>
                                {isRelayGroup ? (<select style={theme_1.s.select} value={selectedModelId} onChange={e => handleModelChange(e.target.value)}>
                                        {(() => {
                        const grouped = {};
                        modelsInGroup.forEach(([id, d]) => {
                            (grouped[d.group] = grouped[d.group] || []).push([id, d]);
                        });
                        return Object.entries(grouped).map(([group, models]) => (<optgroup key={group} label={group}>
                                                    {models.map(([id, d]) => <option key={id} value={id}>{d.label}</option>)}
                                                </optgroup>));
                    })()}
                                    </select>) : (<select style={theme_1.s.select} value={selectedModelId} onChange={e => handleModelChange(e.target.value)}>
                                        {modelsInGroup.map(([id, d]) => <option key={id} value={id}>{d.label}</option>)}
                                    </select>)}
                                {def && <p style={theme_1.s.hint}>{def.note}</p>}
                                {isRelayGroup && (<div style={{ marginTop: '10px' }}>
                                        <label style={theme_1.s.label}>{lang === 'zh' ? '中转平台（点击快速填入）' : 'Relay Platform (click to fill Base URL)'}</label>
                                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginTop: '4px' }}>
                                            {RELAY_PRESETS.map(r => (<button key={r.name} onClick={() => setBaseUrl(r.url)} style={{
                            ...
                            color: baseUrl === r.url ? 'var(--vscode-button-foreground)' : undefined,
                                        ...
                                        {RELAY_PRESETS.map(r => baseUrl === r.url && (<p key={r.name} style={{ ...theme_1.s.hint, marginTop: '6px' }}>
                                                {r.note} — <a href={r.site} style={{ color: 'var(--vscode-textLink-foreground)' }}>注册 / 获取 Key ↗</a>
                                            </p>))}
                                        <div style={{ marginTop: '8px' }}>
                                            <label style={theme_1.s.label}>Base URL</label>
                                            <input style={theme_1.s.input} value={baseUrl} onChange={e => setBaseUrl(e.target.value)} placeholder="https://your-relay.com/v1"/>
                                        </div>
                                    </div>)}
                                {!isRelayGroup && def?.relay && (<p style={{ ...theme_1.s.hint, color: 'var(--vscode-errorForeground)', marginTop: '6px' }}>
                                        此模型需要中转服务，下一步填写中转地址。
                                    </p>)}
                            </div>)}
                        ...
                        {isCustomGroup && (<>
                                <div style={{ marginBottom: '12px' }}>
                                    <label style={theme_1.s.label}>Model ID <span style={{ fontWeight: 400, opacity: 0.7 }}>(接口中使用的模型名称)</span></label>
                                    <input style={theme_1.s.input} value={customModelId} onChange={e => setCustomModelId(e.target.value)} placeholder="e.g. gpt-4o / claude-3-5-sonnet-20241022"/>
                                </div>
                                <div style={{ marginBottom: '12px' }}>
                                    <label style={theme_1.s.label}>显示名称 <span style={{ fontWeight: 400, opacity: 0.7 }}>(在列表里显示的名字)</span></label>
                                    <input style={theme_1.s.input} value={customLabel} onChange={e => setCustomLabel(e.target.value)} placeholder="e.g. 我的专属模型"/>
                                </div>
                            </>)}

                        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px', marginTop: '20px' }}>
                            <button style={theme_1.s.btnSecondary} onClick={onClose}>取消</button>
                            <button style={theme_1.s.btnPrimary} onClick={() => setStep(2)} disabled={!isCustomGroup && !selectedModelId}>
                                下一步
                            </button>
                        </div>
                    </>)}
                ...
                {step === 2 && (<>
                        ...
                        {isEdit && (<div style={{ marginBottom: '14px' }}>
                                <label style={theme_1.s.label}>Model ID <span style={{ fontWeight: 400, opacity: 0.7 }}>{`(可直接修改型号，如 ${GROUP_MODEL_EXAMPLES[selectedGroup] ?? 'glm-5 / glm-4.7'})`}</span></label>
                                <input style={theme_1.s.input} value={isCustomGroup ? customModelId : selectedModelId} onChange={e => isCustomGroup ? setCustomModelId(e.target.value) : setSelectedModelId(e.target.value)} placeholder={GROUP_MODEL_EXAMPLES[selectedGroup] ?? '如 glm-5、deepseek-chat、MiniMax-M2.5'}/>
                            </div>)}

                        <div style={{ marginBottom: '16px' }}>
                            <label style={{ ...theme_1.s.label, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                <span>
                                ...
                                {def?.defaultTasks && (<button style={{ ...theme_1.s.btnSecondary, padding: '2px 8px', fontSize: '10px', marginLeft: '8px' }} onClick={() => setTasks([...(def?.defaultTasks || [])])}>
                                        {lang === 'zh' ? '恢复默认' : 'Reset'}
                                    </button>)}
                            </label>
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginTop: '4px' }}>
                ...
                return (<span key={t.id} onClick={() => toggleTask(t.id)} style={{
                        ...
                        <div style={{ marginBottom: '14px' }}>
                            <label style={theme_1.s.label}>
                                Base URL
                                {def?.relay && (<span style={{ fontWeight: 400, fontSize: '11px', color: 'var(--vscode-errorForeground)', marginLeft: '6px' }}>
                                        此模型需要填写中转服务地址
                                    </span>)}
                            </label>
                            <input style={theme_1.s.input} value={baseUrl} onChange={e => setBaseUrl(e.target.value)} placeholder="https://api.xxx.com/v1"/>
                            <p style={theme_1.s.hint}>
                                {def?.relay
                ...
                : '已自动填入官方地址，如使用中转服务可修改。'}
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '20px' }}>
                            <button style={theme_1.s.btnPrimary} onClick={() => setStep(1)}>上一步</button>
                            <div style={{ display: 'flex', gap: '8px' }}>
                                <button style={theme_1.s.btnSecondary} onClick={onClose}>取消</button>
                                <button style={theme_1.s.btnPrimary} onClick={() => setStep(3)}>下一步</button>
                            </div>
                        </div>
                    </>)}
                ...
                {step === 3 && (<>
                        ...
                        <div style={{ marginBottom: '6px' }}>
                            <label style={theme_1.s.label}>API Key</label>
                            <input type="password" style={theme_1.s.input} value={apiKey} onChange={e => setApiKey(e.target.value)} placeholder="sk-..."/>
                            <p style={theme_1.s.hint}>加密存储，不会明文保存到磁盘。</p>
                        </div>
                ...
                return (<p style={{ ...theme_1.s.hint, marginTop: '10px' }}>
                                    还没有 Key？
                                    <a href={link.url} style={{ color: 'var(--vscode-textLink-foreground)', marginLeft: '4px', cursor: 'pointer' }} onClick={e => { e.preventDefault(); vscode_api_1.vscode.postMessage({ command: 'openUrl', url: link.url }); }}>
                                        {link.text}
                                    </a>
                                </p>);
            })()}
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '24px' }}>
                            <button style={theme_1.s.btnPrimary} onClick={() => setStep(2)}>上一步</button>
                            <div style={{ display: 'flex', gap: '8px' }}>
                                <button style={theme_1.s.btnSecondary} onClick={onClose}>取消</button>
                                <button style={theme_1.s.btnPrimary} onClick={handleSave}>保存</button>
                            </div>
                        </div>
                    </>)}
            </div>
...
const ConfigPanel = ({ lang }) => {
    ...
    const [showPricing, setShowPricing] = (0, react_1.useState)(false);
    ...
    return (<div style={{ maxWidth: '700px', paddingBottom: '40px' }}>
            {/* Header */}
            ...
            : `${enabled} model(s) enabled · Conductor Hub auto-routes each task to the matching model`}
                ...
                <button style={theme_1.s.btnPrimary} onClick={() => { setEditTarget(undefined); setShowModal(true); }}>
                    {lang === 'zh' ? '+ 添加模型' : '+ Add Model'}
                </button>
            </div>

            {/* List */}
            {models.length === 0 ? (<div style={{
                ...
                </div>) : (models.map(m => (<ModelCard key={m.id} model={m} apiKey={apiKeys[m.id] || ''} lang={lang} onEdit={handleEdit} onRemove={handleRemove} onToggle={handleToggle} initialTestResult={testResults[m.id]}/>)))}

            {/* Codex CLI (ChatGPT OAuth) Card */}
            <div style={{ ...theme_1.s.card, marginTop: '18px', borderLeft: `3px solid ${codexStatus?.installed ? '#10A37F' : '#E8740C'}` }}>
            ...
            : '安装后可用 ChatGPT Plus 账号直接调用，无需 API Key'}
                    ...
                    <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
                        {!codexStatus?.installed && (<button style={{ ...theme_1.s.btnPrimary, fontSize: '11px', padding: '4px 10px' }} onClick={() => vscode_api_1.vscode.postMessage({ command: 'openTerminalWithCmd', cmd: 'npm install -g @openai/codex && codex login' })}>
                                安装并登录
                            </button>)}
                        {codexStatus?.installed && (<button style={{ ...theme_1.s.btnSecondary, fontSize: '11px', padding: '4px 10px' }} onClick={() => vscode_api_1.vscode.postMessage({ command: 'openTerminalWithCmd', cmd: 'codex login' })}>
                                重新登录
                            </button>)}
                        <button style={{ ...theme_1.s.btnSecondary, fontSize: '11px', padding: '4px 10px' }} onClick={() => vscode_api_1.vscode.postMessage({ command: 'openUrl', url: 'https://github.com/openai/codex' })}>
                            文档
                        </button>
                    </div>
                </div>
                {codexStatus?.installed && codexStatus.loggedIn && (<div style={{ textAlign: 'right', marginTop: '6px', fontSize: '11px', color: '#10A37F', fontWeight: 500 }}>✅ 已连通</div>)}
            </div>

            {/* Gemini CLI Card */}
            <div style={{ ...theme_1.s.card, marginTop: '12px', borderLeft: `3px solid ${geminiStatus?.installed ? '#4285F4' : '#E8740C'}` }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                        <span style={{ fontWeight: 600, fontSize: '13px' }}>
            ...
            : '安装后可用 Google 账号本地凭据调用，无需 API Key'}
                    ...
                    <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
                        {!geminiStatus?.installed && (<button style={{ ...theme_1.s.btnPrimary, fontSize: '11px', padding: '4px 10px' }} onClick={() => vscode_api_1.vscode.postMessage({ command: 'openTerminalWithCmd', cmd: 'npm install -g @google/gemini-cli && gemini' })}>
                                安装并登录
                            </button>)}
                        {geminiStatus?.installed && (<button style={{ ...theme_1.s.btnSecondary, fontSize: '11px', padding: '4px 10px' }} onClick={() => vscode_api_1.vscode.postMessage({ command: 'openTerminalWithCmd', cmd: 'gemini' })}>
                                重新登录
                            </button>)}
                        <button style={{ ...theme_1.s.btnSecondary, fontSize: '11px', padding: '4px 10px' }} onClick={() => vscode_api_1.vscode.postMessage({ command: 'openUrl', url: 'https://github.com/google/gemini-cli' })}>
                            文档
                        </button>
                    </div>
                </div>
                {geminiStatus?.installed && geminiStatus.loggedIn && (<div style={{ textAlign: 'right', marginTop: '6px', fontSize: '11px', color: '#4285F4', fontWeight: 500 }}>✅ 已连通</div>)}
            </div>
            ...
            display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                ...
                borderRadius: '4px', overflow: 'hidden',
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                            ...
                            <tbody>
                                {PRICE_TABLE.map((row, i) => (<tr key={row.id} style={{
                    ...
                    borderTop: '1px solid var(--vscode-input-border)',
                                        ...
                                        <td style={{ padding: '5px 10px', color: theme_1.providerColors[row.group] || 'inherit', fontWeight: 500 }}>{row.group}</td>
            ...
            <div style={{ marginTop: '18px' }}>
                <button onClick={() => setShowRouting(!showRouting)} style={{
            ...theme_1.s.btnSecondary,
            width: '100%', textAlign: 'left',
            display: 'flex', justifyContent: 'space-between', alignItems: 'center',
        }}>
            ...
            {showModal && (<AddEditModal lang={lang} existing={editTarget} onSave={handleSave} onClose={() => { setShowModal(false); setEditTarget(undefined); }}/>)}
        </div>);
};
