...
import { colors, radius, glass, providerColors } from '../theme';
...
interface OverviewStats {
    ...
    avgLatency: number;
    totalTokens: number;
    tokensByProvider: TokenByProvider[];
    recentRequests: {
        id: string;
        timestamp: number;
        method: string;
...
const TokenByProviderChart: React.FC<{ data: TokenByProvider[]; lang: Lang }> = ({ data, lang }) => {
...
const OverviewPanel: React.FC<{ lang: Lang; onSwitchTab: (tab: string) => void }> = ({ lang, onSwitchTab }) => {
    ...
    return (
        <div style={{ maxWidth: '880px' }}>
            ...
            {stats.tokensByProvider.length > 0 && (
