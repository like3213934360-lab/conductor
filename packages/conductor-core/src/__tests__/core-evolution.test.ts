/**
 * Conductor AGC — 核心测试套件
 *
 * 覆盖矩阵:
 * 1. DagEngine — 正常流、并发限制、终止判定
 * 2. RiskRouter — 四级路由、快速通道、BLOCK 升级
 * 3. GovernanceGateway — 4 拦截点、failFast、Trust-Weighted Release
 * 4. EmbeddingCollusionDetector — 余弦相似度、降级、批量检测
 * 5. SnapshotStore — 保存/查找/修剪、SnapshotManager 增量回放
 *
...
// ── 3. Collusion Detector Tests ──────────────────────────────────────

describe('EmbeddingCollusionDetector', () => {
  let cosineSimilarity: any
  let jaccardSimilarity: any
  let EmbeddingCollusionDetector: any

  beforeEach(async () => {
    const mod = await import('../collusion/embedding-detector.js')
    cosineSimilarity = mod.cosineSimilarity
    jaccardSimilarity = mod.jaccardSimilarity
    EmbeddingCollusionDetector = mod.EmbeddingCollusionDetector
  })
  ...
  describe('jaccardSimilarity', () => {
    ...
    it('完全不同文本应返回 0', () => {
      expect(jaccardSimilarity('cat dog', 'xyz abc')).toBe(0)
    })
  ...
  describe('detect', () => {
    it('高相似度 embedding 应返回 FLAGGED', async () => {
      const mockProvider = {
        embed: vi.fn()
          .mockResolvedValueOnce({ embedding: [1, 0, 0], model: 'test', tokens: 10 })
          .mockResolvedValueOnce({ embedding: [0.99, 0.01, 0], model: 'test', tokens: 10 }),
        isAvailable: () => true,
      }
      const detector = new EmbeddingCollusionDetector(mockProvider, { flaggedThreshold: 0.90 })
      const result = await detector.detect('text a', 'text b')
      expect(result.similarity).toBeGreaterThan(0.90)
      expect(result.status).toBe('FLAGGED')
      expect(result.method).toBe('embedding')
    })

    it('API 失败时应降级到 Jaccard', async () => {
      const mockProvider = {
        embed: vi.fn().mockRejectedValue(new Error('API error')),
        isAvailable: () => true,
      }
      const detector = new EmbeddingCollusionDetector(mockProvider, { fallbackToJaccard: true })
      const result = await detector.detect('same words', 'same words')
      expect(result.method).toBe('jaccard_fallback')
      expect(result.similarity).toBeGreaterThan(0)
    })

    it('provider 不可用且无降级应抛出', async () => {
      const mockProvider = {
        embed: vi.fn(),
        isAvailable: () => false,
      }
      const detector = new EmbeddingCollusionDetector(mockProvider, { fallbackToJaccard: false })
      await expect(detector.detect('a', 'b')).rejects.toThrow('unavailable')
    })
  })
})
...
describe('InMemorySnapshotStore', () => {
  ...
  beforeEach(async () => {
    const mod = await import('../../../conductor-shared/src/state/snapshot-store.js')
