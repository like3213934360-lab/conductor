 ...
 * - loopStates 通过 AGCState 传入 (不在引擎中保持状态)
 * - 所有循环变更通过事件表达
 */
export class CyclicDagEngine {
  ...
  advance(
  ...
  ): DagExecutionPatch & { loopEvents: AGCEventEnvelope[]; updatedLoopStates: LoopStateStore } {
    const edges = graph.edges as GraphEdgeEx[]
    ...
    const allEvents: AGCEventEnvelope[] = []
    const loopEvents: AGCEventEnvelope[] = []
    ...
    for (const loopEdge of loopEdges) {
      ...
      if (decision.continue) {
        ...
        currentVersion++
        const iterEvent: AGCEventEnvelope = {
        ...
        currentVersion++
        const endEvent: AGCEventEnvelope = {
          eventId: createEventId(),
          runId: state.runId,
          type,
          payload: {
