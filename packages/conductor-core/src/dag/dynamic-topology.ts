/**
 * Conductor AGC — 动态拓扑扩展 (Dynamic Topology Extensions)
 *
 * AGC v8.0: 在现有固定 DAG 上叠加拓扑覆盖层，支持子图、受控循环、运行时节点注入。
 *
 * 设计参考:
 ...
 *
 * 向后兼容: 所有新字段 optional，legacy DAG 无需修改即可运行。
 */

import type { RunGraph, GraphNode, GraphEdge, AGCState, AGCEventEnvelope } from '@anthropic/conductor-shared'
import { createEventId, createISODateTime } from '@anthropic/conductor-shared'

// ── 扩展类型 (全部 optional，向后兼容) ─────────────────────────────────────────
