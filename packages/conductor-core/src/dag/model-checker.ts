/**
 * Conductor AGC — 轻量级模型检查器 (Lightweight Bounded Model Checker)
 *
 * AGC v8.0: 在 formal-verifier 运行时检查基础上，
...
import type { RunGraph, GraphNode, GraphEdge, AGCState } from '@anthropic/conductor-shared'
...
export class BoundedModelChecker {
  ...
  private checkInvariantAt(
    ...
    const prevState = this.toMockAGCState({ ...state, version: Math.max(0, state.version - 1) })
    const nextState = this.toMockAGCState(state)
  ...
  /** 构造模拟 AGCState */
  private toMockAGCState(state: StateSnapshot): AGCState {
