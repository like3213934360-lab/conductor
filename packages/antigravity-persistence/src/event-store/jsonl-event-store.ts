/**
 * Antigravity Workflow Runtime — JSONL 文件 EventStore
 *
 * 审查修复:
...
export class JsonlEventStore implements EventStore {
  ...
  async append(input: AppendEventsInput): Promise<void> {
    ...
    try {
      ...
      if (expectedVersion === 'no_stream') {
        if (currentVersion > 0) {
          throw new WorkflowError(
            WorkflowErrorCode.STATE_VERSION_CONFLICT,
      ...
      const lines = events.map((event: WorkflowEventEnvelope) => JSON.stringify(event))
  ...
  async *loadStream(query: LoadEventsQuery): AsyncIterable<WorkflowEventEnvelope> {
    ...
    for await (const event of readJsonlStream<WorkflowEventEnvelope>(streamPath)) {
  ...
  async getCurrentVersion(runId: string): Promise<number> {
    const streamPath = this.getStreamPath(runId)
    const lastEvent = await readLastJsonlLine<WorkflowEventEnvelope>(streamPath)
