/**
 * Antigravity Workflow Runtime — DAG 拓扑工具
 *
 * 提供环检测、拓扑排序和就绪节点发现。
 * 参考 Kahn 算法（BFS 拓扑排序）。
 *
...
import type { RunGraph, GraphNode, GraphEdge } from '@anthropic/antigravity-shared'
...
export function topologicalSort(graph: RunGraph): string[] {
  if (graph.nodes.length === 0) {
    throw new WorkflowError(WorkflowErrorCode.GRAPH_EMPTY, 'DAG 图为空，至少需要一个节点')
  }
  ...
  if (sorted.length !== graph.nodes.length) {
    ...
    throw new WorkflowError(
      WorkflowErrorCode.GRAPH_CYCLE_DETECTED,
