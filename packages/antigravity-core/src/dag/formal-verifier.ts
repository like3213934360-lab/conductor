/**
 * Antigravity Workflow Runtime — 形式化验证中间件 (Formal State Invariant Verifier)
 *
 * Workflow Runtime vNext: 在 EventStore append 前验证状态转换不变量。
 *
 * 设计参考:
...
import type { WorkflowState, RunGraph, WorkflowEventEnvelope } from '@anthropic/antigravity-shared'
...
export interface TransitionProperty {
  ...
  description: string
  /** 断言函数 — 违反时 throw */
  assert(graph: RunGraph, prevState: WorkflowState, patch: DagExecutionPatch): void
}
...
export class StateInvariantVerifier {
   ...
   */
  verifyPatch(graph: RunGraph, prevState: WorkflowState, patch: DagExecutionPatch): void {
