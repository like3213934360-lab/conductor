/**
 * Antigravity Workflow Runtime — 动态信任因子服务 (Trust Factor Service)
 *
 * Workflow Runtime vNext: 替代固定权重 (0.30/0.35/0.35) 的动态信任评分系统。
 *
 * 设计参考:
...
export class TrustFactorService {
   ...
   * 记录模型调用结果
   */
  recordModelCall(modelId: string, success: boolean, durationMs: number): void {
    if (!this.modelHistory.has(modelId)) {
      this.modelHistory.set(modelId, [])
    }
    const records = this.modelHistory.get(modelId)!
    records.push({ modelId, success, durationMs, timestamp: Date.now() })
    // 保留最近 100 条
  ...
  private computeSignals(params: {
    ...
    if (params.modelId) {
      const records = this.modelHistory.get(params.modelId) ?? []
      if (records.length > 0) {
