/**
 * Antigravity Workflow Runtime — Event Sourcing 事件定义
 *
 * 审查修复 #1: 事件 payload 强类型 discriminated union
 *
 * 设计:
 * - WorkflowEventEnvelopeSchema 的 payload 使用 z.record，保证 schema-first 序列化
 * - TypeScript 层使用 WorkflowEvent discriminated union 做强类型保护
 * - WorkflowTypedEventEnvelope 替代 payload as xxx 断言
 */
import { z } from 'zod'
...
/** 节点失败事件 */
export interface NodeFailedPayload { nodeId: string; error: string; leaseId?: string }
...
export interface NodeSkippedPayload { nodeId: string; reason: string; leaseId?: string }
...
/** 所有 workflow 事件的联合类型 (discriminated union on 'type') */
 ...
 * 用法:
 * ```ts
 * function handleEvent(env: WorkflowTypedEventEnvelope) {
 *   switch (env.type) {
 ...
 * ```
 */
export type WorkflowTypedEventEnvelope = WorkflowEventEnvelope & WorkflowEvent
...
const NodeStartedPayloadSchema = z.object({
  ...
  payload: z.object({
    ...
    allowedModelPool: z.array(z.string()),
    inputDigest: z.string().optional(),
  }),
})

const NodeHeartbeatPayloadSchema = z.object({
...
const NodeFailedPayloadSchema = z.object({
  ...
  payload: z.object({ nodeId: z.string(), error: z.string(), leaseId: z.string().optional() }),
})

const NodeSkippedPayloadSchema = z.object({
  type: z.literal('NODE_SKIPPED'),
  payload: z.object({ nodeId: z.string(), reason: z.string(), leaseId: z.string().optional() }),
})
...
/**
 * workflow 事件 Zod discriminatedUnion — 运行时强制校验
 *
 * 用于反序列化时替代 as unknown as 断言。
 * 拒绝结构不匹配的恶意/损坏事件。
 */
export const WorkflowTypedEventSchema = z.discriminatedUnion('type', [
...
/**
 * 安全解析 workflow 事件 — 科研升级 C
 *
 * 用法:
 * ```ts
 * const raw = JSON.parse(line)
 * const event = parseWorkflowEvent(raw)
 * if (event) {
 *   switch (event.type) {
 ...
 */
export function parseWorkflowEvent(raw: unknown): WorkflowTypedEventEnvelope | null {
  // 第一步: 校验信封结构 (eventId, runId, version, timestamp)
  const envelope = WorkflowEventEnvelopeSchema.safeParse(raw)
  if (!envelope.success) return null

  // 第二步: 校验事件类型 + payload 结构
  const typed = WorkflowTypedEventSchema.safeParse({
