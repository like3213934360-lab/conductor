...
class DashboardPanel {
    static createOrShow(extensionUri, storage, settings, onConfigChanged) {
        const column = vscode.window.activeTextEditor?.viewColumn;
        if (DashboardPanel.currentPanel) {
            DashboardPanel.currentPanel._panel.reveal(column);
            return;
        }
        const panel = vscode.window.createWebviewPanel('conductorDashboard', 'Conductor Hub', column || vscode.ViewColumn.One, {
            enableScripts: true,
            localResourceRoots: [
    ...
    constructor(panel, extensionUri, storage, settings, onConfigChanged) {
        this.storage = storage;
        this.settings = settings;
        this._disposables = [];
        this._modelTestCache = new Map();
        this._panel = panel;
        ...
        this._panel.webview.onDidReceiveMessage(async (message) => {
            switch (message.command) {
                ...
                case 'testConnection': {
                    ...
                    try {
                        ...
                        if (res.ok) {
                            ...
                            this._panel.webview.postMessage({ command: 'testResult', requestId, ok: false, msg: err.substring(0, 70) });
                        }
                    }
                    catch (e) {
                ...
                case 'generateDiagnostics': {
                    ...
                    const codexVer = (0, child_process_1.spawnSync)('codex', ['--version'], { encoding: 'utf8', timeout: 3000, shell: true });
                    const geminiVer = (0, child_process_1.spawnSync)('gemini', ['--version'], { encoding: 'utf8', timeout: 3000, shell: true });
                    const report = [
                        '## Conductor Hub 诊断报告',
                ...
                // ── CLI status ────────────────────────────────────────────
                case 'getCodexStatus': {
                    const ver = (0, child_process_1.spawnSync)('codex', ['--version'], { encoding: 'utf8', timeout: 5000, shell: true });
                    if (ver.error) {
                ...
                case 'getGeminiStatus': {
                    const gVer = (0, child_process_1.spawnSync)('gemini', ['--version'], { encoding: 'utf8', timeout: 5000, shell: true });
                    if (gVer.error) {
    ...
    async _handleOverviewStats() {
        ...
        for (const m of allModels) {
            ...
            modelStatuses.push({
                ...
                group: m.group || '', enabled: m.enabled !== false,
        ...
        try {
            ...
            if (fs.existsSync(codexDbPath)) {
                ...
                const sqlResult = (0, child_process_1.spawnSync)('sqlite3', [
                    codexDbPath,
                    '-separator', '|',
    ...
    /** 收集所有模型的 API Key */
    async _collectModelApiKeys(models) {
        const apiKeys = {};
