...
import { s, colors, radius, shadow, modelFamilyColors } from '../theme';
...
/** Example model IDs shown in the Model ID input per model family */
...
// Model-family colors imported from theme.ts


// ─── Shared Styles ────────────────────────────────────────────────────────────

// Styles imported from '../theme' — s, colors, radius, shadow, modelFamilyColors
    ...
    return (
        <div style={{
            ...
            position: 'relative' as const,
                    ...
                    {/* Row 1: Model family info + actions */}
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flex: 1, minWidth: 0 }}>
                    {/* Model-family color dot */}
                    <div style={{
                        width: '10px', height: '10px', borderRadius: '50%', flexShrink: 0,
                        background: modelFamilyColors[def?.group || ''] || colors.brand,
                        boxShadow: `0 0 8px ${modelFamilyColors[def?.group || ''] || colors.brand}60`,
                    }} />
                    <span style={{ fontWeight: 600, fontSize: '13px', letterSpacing: '-0.2px' }}>{def?.group || model.modelId}</span>
                    <span style={{
                        fontSize: '12px', fontWeight: 500,
                        color: modelFamilyColors[def?.group || ''] || colors.brand,
                        opacity: 0.9,
                    }}>{model.label}</span>
                </div>
...
const AddEditModal: React.FC<{
    ...
    const handleGroupChange = (g: string) => {
        ...
        // For relay group, pick first available model from all model families
                ...
                {step === 1 && (
                    <>
                        ...
                        <div style={{ marginBottom: '16px' }}>
                            <label style={{ ...s.label, marginBottom: '10px' }}>{lang === 'zh' ? '选择模型家族' : 'Select Model Family'}</label>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px', marginBottom: '12px' }}>
                                {GROUPS.slice(0, 5).map(g => {
                                    ...
                                    const clr = modelFamilyColors[g] || colors.brand;
                                    return (
                            ...
                            <div style={{ marginBottom: '14px' }}>
                                ...
                                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginTop: '6px' }}>
                                    {(isRelayGroup
                                        ...
                                        const clr = modelFamilyColors[d.group] || colors.brand;
                                        return (
...
const ConfigPanel: React.FC<ConfigPanelProps> = ({ lang }) => {
    ...
    const [arktsLspStatus, setArktsLspStatus] = useState<ArktsLspStatus | null>(null);
    ...
    useEffect(() => {
        const handler = (ev: MessageEvent) => {
            ...
            if (ev.data.command === 'loadArktsLspStatus' && ev.data.status) {
                setArktsLspStatus(ev.data.status as ArktsLspStatus);
            }
        };
        window.addEventListener('message', handler);
        ...
        vscode.postMessage({ command: 'getArktsLspStatus' });
                    ...
                    <div style={{ minWidth: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
                            <div style={{
                                ...
                                background: arktsLspStatus?.state === 'running'
                                    ? colors.success
                                    : arktsLspStatus?.state === 'starting'
                                        ? colors.brand
                                        : arktsLspStatus?.state === 'error'
                                            ? colors.error
                                            : 'var(--vscode-descriptionForeground)',
                                boxShadow: arktsLspStatus?.state === 'running'
                                    ? `0 0 8px ${colors.success}66`
                                    : arktsLspStatus?.state === 'starting'
                                        ? `0 0 8px ${colors.brand}66`
                                        : arktsLspStatus?.state === 'error'
                                            ? `0 0 8px ${colors.error}66`
                                            : 'none',
                            }} />
                            <span style={{ fontWeight: 700, fontSize: '13px' }}>
                                {lang === 'zh' ? 'ArkTS LSP（可选附加功能）' : 'ArkTS LSP (Optional)'}
                            </span>
                        </div>
                        <div style={{ fontSize: '12px', color: 'var(--vscode-descriptionForeground)', opacity: 0.8, lineHeight: '1.6' }}>
                            {arktsLspStatus?.message || (lang === 'zh' ? '正在读取 ArkTS LSP 状态…' : 'Loading ArkTS LSP status...')}
                        </div>
                        <div style={{ fontSize: '11px', color: 'var(--vscode-descriptionForeground)', opacity: 0.6, marginTop: '6px' }}>
                            {lang === 'zh'
                                ? `DevEco 检测: ${arktsLspStatus?.devecoDetected ? '已发现' : '未发现'}`
                                : `DevEco: ${arktsLspStatus?.devecoDetected ? 'detected' : 'not detected'}`}
                            {arktsLspStatus?.workspaceRoot ? ` · ${arktsLspStatus.workspaceRoot}` : ''}
                        </div>
                    </div>
                    <button
                        style={{
                            ...(arktsLspStatus?.enabled ? s.btnSecondary : s.btnPrimary),
                            minWidth: '110px',
                            borderRadius: radius.pill,
                            padding: '8px 16px',
                            fontSize: '12px',
                        ...
                        onClick={() => vscode.postMessage({
                            ...
                            enabled: !(arktsLspStatus?.enabled === true),
                        })}
                    >
                        {arktsLspStatus?.enabled
                            ? (lang === 'zh' ? '关闭 LSP' : 'Disable LSP')
                            : (lang === 'zh' ? '开启 LSP' : 'Enable LSP')}
                    </button>
                </div>
            </div>
