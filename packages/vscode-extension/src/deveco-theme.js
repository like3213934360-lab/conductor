...
exports.applyDevEcoColors = applyDevEcoColors;
...
function applyDevEcoColors() {
    const config = vscode_1.workspace.getConfiguration('editor');
    applyTextMateColors(config);
    applySemanticColors(config);
}
function applyTextMateColors(config) {
    const inspection = config.inspect('tokenColorCustomizations');
    const userGlobal = inspection?.globalValue;
    const existingRules = (userGlobal?.textMateRules || []);
    const hasSentinel = existingRules.some((r) => r.scope === TEXTMATE_SENTINEL_SCOPE && r.settings?.foreground === '#CC7832');
    if (hasSentinel)
        return;
    const filteredRules = existingRules.filter((r) => typeof r.scope !== 'string' || !r.scope.endsWith('.ets'));
    const mergedRules = [...filteredRules, ...DEVECO_TEXTMATE_RULES];
    config.update('tokenColorCustomizations', { ...(userGlobal || {}), textMateRules: mergedRules }, vscode_1.ConfigurationTarget.Global).then(undefined, () => { });
}
function applySemanticColors(config) {
    const inspection = config.inspect('semanticTokenColorCustomizations');
    const userGlobal = inspection?.globalValue;
    const currentRules = (userGlobal?.rules || {});
    ...
    config.update('semanticTokenColorCustomizations', { ...(userGlobal || {}), enabled: true, rules: mergedRules }, vscode_1.ConfigurationTarget.Global).then(() => { config.update('semanticHighlighting.enabled', true, vscode_1.ConfigurationTarget.Global); }, () => { });
}
//# sourceMappingURL=deveco-theme.js.map