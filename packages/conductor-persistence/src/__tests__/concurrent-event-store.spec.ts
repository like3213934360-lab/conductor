...
describe('EventStore 并发写入', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'conductor-concurrent-'))
  ...
  /** 创建测试事件 (版本号由调用者指定) */
  function createEvent(runId: string, version: number, seq: number): AGCEventEnvelope {
    return {
  ...
  async function appendSequential(runId: string, count: number): Promise<void> {
    for (let i = 0; i < count; i++) {
      ...
      const nextVersion = events.length
      await store.append({
        runId,
  ...
  it('顺序 append 同一 runId — 版本连续', async () => {
    ...
    for (let i = 0; i < events.length; i++) {
      expect(events[i]!.version).toBe(i)
    }
  })
  ...
  it('并发 load — 读取不应互相干扰', async () => {
    ...
    // 并发 10 次 load
    const loadResults: AGCEventEnvelope[][] = []
    const promises = Array.from({ length: 10 }, () =>
      store.load({ runId }).then((events: AGCEventEnvelope[]) => {
  ...
  it('互斥锁竞争 — 版本冲突正确拒绝', async () => {
    ...
    // 然后尝试写入 version: 0 的事件
    // 由于互斥锁串行执行, 只有第一个能成功
    // 其余因 event.version(0) != startVersion(1,2,...) 而失败
    // 这验证了 EventStore 的版本完整性保证
    const promises = Array.from({ length: concurrency }, (_: unknown, i: number) =>
      store.append({
        runId,
        events: [createEvent(runId, 0, i)],
        expectedVersion: 'any',
      }),
    )
    ...
    // 只有第一个写入者成功 (version: 0 匹配空流的 startVersion: 0)
