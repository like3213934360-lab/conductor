...
- 构建与验证：`npm test`、`npm run smoke:daemon`、`npm run smoke:mcp` 已通过
- 当前总体验收结论：**未通过，仍存在实质性未闭环问题**
- 已进入默认主链：`AuthorityRuntimeKernel`、daemon-owned transition/skip authority、`UpcastingEventStore` 默认读路、callback hardening、strict trust mode、`RuntimeTelemetrySink` hook surface
- 未进入默认主链：`GovernanceGateway` 默认治理入口、durable domain-event dual-write、`VerificationSnapshot` / `proofGraphDigest` production artifact chain、`CyclicDagEngine`
...
| 治理 | policy engine + bootstrap preflight + stage-tagged verdicts | 存疑 | `GovernanceGateway` 未成为默认治理入口 |
| Event Sourcing / Replay | JSONL event log + checkpoint + `UpcastingEventStore` 读路 | 基本通过 | daemon domain events 尚未成为 durable canonical source |
...
| 信任治理 | Trust Registry + callback hardening + strict trust mode | 通过 | 签名与委派边界真实生效 |
...
| 发布产物 | trace bundle + attestation / dossier / bundle / certification 基础链 | 基本通过 | `VerificationSnapshot` / `proofGraphDigest` 未完成生产主链接线 |
...
│(default)│(DR/4L)│ (partial) │ (A2A-style)   │
├────────┴────────┴───────────┴───────────────┤
...
│   ├── governance/         #   治理原语 + GovernanceGateway（默认 cutover 未完成）
│   ├── risk/               #   DR 风险路由
...
release 阶段默认走 evidence gate + policy pack：节点执行完成并不等于允许发布，daemon 会基于导出的 policy-as-code 规则检查 receipt、artifact、judge signal 和 handoff 证据后再决定 `completed`、`paused_for_human` 或 `policy_skipped`。当前治理主链是 `policyEngine + pure evaluator helper` 的混合态；`GovernanceGateway` 尚未成为默认 runtime 治理入口，因此这里不能再表述为“统一 GaaS 主链已经完成”。
...
`VerificationSnapshot`、`ArtifactReference` 和 `proofGraphDigest` 的 schema / builder / verifier 已经存在，但它们尚未形成默认生产 artifact 主链。当前生产入口仍主要基于 `run` / `verifyRun` 摘要直接构造终态 artifacts，因此不能把这部分能力描述为“已完全主链化的 proof graph / verification snapshot chain”。
