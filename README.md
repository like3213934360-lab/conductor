...
| 🔍 可观测性 | Structural Tracer *(experimental: OTel SDK 未接入)* | LangSmith |
| 🧠 记忆系统 | Keyword Recall + Snapshot *(experimental)* | MemGPT |
...
| 🧪 基准评估 | DAG + 治理评估套件 *(experimental harness)* | AgentBench |
...
│   (JSONL+Snapshot)  │    Gate     │  (Tracer, exp)       │
├──────────────────────────────────────────────────────────┤
...
│   └── ...                 #   核心运行时子模块
├── antigravity-persistence # 持久化 (JSONL + Keyword Memory)
