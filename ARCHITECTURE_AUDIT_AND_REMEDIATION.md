...
项目最大的结构问题是双轨制。真实 authority 主链在 `antigravity-daemon`，但 `GovernanceGateway`、`CyclicDagEngine`、`UpcastingEventStore`、`DagEngineTracer`、`VectorMemory`、formal spec 等大量能力没有进入 daemon 主路径，却在文档层被描述为已落地主能力。
...
|---|---|---|---|---|
| cyclic DAG | README 将其表述为 `7 节点状态机 + 循环 DAG` | 仓库中存在 `CyclicDagEngine` 与 loop event 建模，但 daemon 主路径仍实例化普通 `DagEngine`，默认 run 不使用 loop semantics | 否 | 已有实验实现，未进入 authority runtime 主链，当前应标记为 `experimental DAG extension` |
| GaaS 4 interception points / PDP / PEP | README 与 `core` 中表述为 `preflight / authorize / observe / release` 四拦截点 | `GovernanceGateway` 在 `core` 中具备设计与测试，但 daemon 主路径实质上只使用 bootstrap preflight，其余阶段主要走 `DaemonPolicyEngine.evaluateX` 的专用分支 | 部分 | 治理能力真实存在，但当前是“policy engine + 部分 gateway”混合态，不应表述为已完成的统一 GaaS 主链 |
| Event Sourcing + Upcasting + Snapshot | README 表述为 `不可变事件流 + Replayable Ledger + Upcasting+Snap` | 事件流和 checkpoint 真实存在；但 receipts / handoffs / verdicts / completion sessions / artifact 状态主要仍由 ledger side tables 承载；upcasting 框架存在但未接入默认 daemon 读路径 | 部分 | 当前是“事件流为运行状态基底，完整系统语义仍非纯 event-sourced”；upcasting 处于待接线状态 |
