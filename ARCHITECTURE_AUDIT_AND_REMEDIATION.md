...
本次整改在实现层面已经大体落地，但按当前真实代码状态，**最终总验收结论仍为：未通过，仍存在实质性未闭环问题**。
...
当前阻断正式归档的关键未闭环点有 3 个：

1. `GovernanceGateway` 尚未成为 runtime 默认治理入口
2. PR-07 的 domain-event dual-write 仍是内存日志，且未覆盖多数治理 verdict
3. `VerificationSnapshot` / `proofGraphDigest` 尚未进入生产 artifact 主链

问题分级：

- `P0`：治理主链未真正切换；verification snapshot / proof graph 仍是 scaffolding；domain-event dual-write 既不 durable 也不完整
- `P1`：README / capability classification 曾把未主链化能力写成 stable/mainline；多处测试主要验证 helper/source-check，而不是默认主链
- `P2`：MCP tool catalog 与 formal capability 注释口径仍需继续统一

归档建议：

- 当前**不建议归档**
- 需要先补实上述 3 个关键未闭环点，再重新进行最终验收
...
第三个核心问题是治理链没有真正进入关键路径。bootstrap 使用 `GovernanceGateway` 做 preflight，但 runtime 主路径在大量阶段仍直接调用 `DaemonPolicyEngine` 的特定 evaluator。结果是 policy-as-code 是真的，GaaS/PDP-PEP 是半真半假。
...
| GaaS 4 interception points / PDP / PEP | README 与 `core` 中一度表述为 `preflight / authorize / observe / release` 四拦截点 | `GovernanceGateway` 在 `core` 中具备设计与测试，bootstrap preflight 也会实例化它；但 daemon runtime 默认治理路径仍不是 `GovernanceGateway` orchestrator，而是 direct policy engine / pure evaluator helper 的混合态 | 部分 | 治理能力真实存在，但当前仍不能表述为已完成的统一 GaaS 主链 |
| Event Sourcing + Upcasting + Snapshot | README 曾表述为 `不可变事件流 + Replayable Ledger + Upcasting+Snap` | 工作流事件流、checkpoint 与 `UpcastingEventStore` 默认读路径真实存在；但 daemon domain events 仍是内存 dual-write，receipts / handoffs / verdicts / completion sessions / artifact 状态主要仍由 ledger side tables 承载，`VerificationSnapshot` builder 也未进入生产 artifact 主链 | 部分 | 当前更准确的定位是“工作流事件流 + upcasting 读路已成立，但关键治理 / artifact 语义仍非纯 event-sourced，verification snapshot 仍处于 scaffolding 阶段” |
...
- 所有终态 artifact 从同一 frozen snapshot 生成。
...
3. 让所有终态 artifact 从该 snapshot 生成。
...
- 所有终态 artifact 引用同一 verification snapshot。
