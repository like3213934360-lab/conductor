#!/usr/bin/env node
/**
 * VERIFICATION TEST: languageId = "deveco.apptool.ets"
 * This should fix hover, definition, and all other features!
 */
...
async function main() {
    console.log('🔬 VERIFICATION: languageId = "deveco.apptool.ets"')
    ...
    // ========== didOpen with CORRECT languageId ==========
    console.log('\n📂 didOpen (languageId: deveco.apptool.ets)...')
    await aceReq('aceProject/onAsyncDidOpen', {
        textDocument: { uri, languageId: ACE_LANG.ets, version: 1, text: content }
    })
    await sleep(3000)
    ...
    console.log(`\n📌 1. Hover Column() at L${tl}:${tc}`)
    let hr = await aceReq('aceProject/onAsyncHover', { textDocument: { uri }, position: { line: tl, character: tc } })
    console.log(`   Result: ${JSON.stringify(hr).substring(0, 300)}`)
    console.log(`   ✅ Has content: ${hr?.contents?.length > 0}`)

    // Test 2: pageStack variable
    for (let i = 0; i < lines.length; i++) {
        if (lines[i].includes('this.pageStack')) {
            const c = lines[i].indexOf('pageStack')
            console.log(`\n📌 2. Hover pageStack at L${i}:${c}`)
    ...
    // Test 3: @Entry decorator
    console.log(`\n📌 3. Hover @Entry at L0:1`)
    hr = await aceReq('aceProject/onAsyncHover', { textDocument: { uri }, position: { line: 0, character: 1 } })
    console.log(`   Result: ${JSON.stringify(hr).substring(0, 300)}`)
    console.log(`   ✅ Has content: ${hr?.contents?.length > 0}`)

    // Test 4: import statement
    for (let i = 0; i < lines.length; i++) {
        if (lines[i].includes('import')) {
            const c = lines[i].indexOf('import') + 7
            console.log(`\n📌 4. Hover import at L${i}:${c}`)
            hr = await aceReq('aceProject/onAsyncHover', { textDocument: { uri }, position: { line: i, character: c } })
            console.log(`   Result: ${JSON.stringify(hr).substring(0, 300)}`)
            console.log(`   ✅ Has content: ${hr?.contents?.length > 0}`)
            break
    ...
    console.log(`\n📌 5. Definition of Column() at L${tl}:${tc}`)
    let dr = await aceReq('aceProject/onAsyncDefinition', { textDocument: { uri }, position: { line: tl, character: tc } })
    console.log(`   Result: ${JSON.stringify(dr).substring(0, 300)}`)
    console.log(`   ✅ Has locations: ${Array.isArray(dr) && dr.length > 0}`)

    // ========== COMPLETION TEST (control) ==========
    console.log('\n' + '='.repeat(60))
    console.log('COMPLETION TEST (control):')
    console.log('='.repeat(60))

    console.log(`\n📌 6. Completion after Column at L${tl}:${tc + 7}`)
    let cr = await aceReq('aceProject/onAsyncCompletion', {
        textDocument: { uri }, position: { line: tl, character: tc + 7 }, context: { triggerKind: 2, triggerCharacter: '.' }
    })
    console.log(`   Items: ${cr?.items?.length || 0}`)
    if (cr?.items?.length > 0) console.log(`   First 5: ${cr.items.slice(0, 5).map(i => i.label).join(', ')}`)
