#!/usr/bin/env node
/**
 * v4: Direct patch of worker/index.js (backup & restore)
 */
...
if (!fs.existsSync(WORKER_BAK)) {
    ...
    console.log('✅ Worker backed up')
} else {
    console.log('✅ Backup already exists')
}

// Read and patch worker
...
try{
  var _dbgFs=require("fs");
  _dbgFs.appendFileSync("${DBG_LOG}","[HOVER] uri="+kt.uri+" langId="+kt.languageId+"\\n");
  _dbgFs.appendFileSync("${DBG_LOG}","[HOVER] getLS="+(Xt?("has-onHover="+!!Xt.onHover+" class="+(Xt.constructor?.name||"?")):"NULL")+"\\n");
  _dbgFs.appendFileSync("${DBG_LOG}","[HOVER] getCtx="+(Qt?JSON.stringify(Qt).substring(0,300):"NULL")+"\\n");
  if(Xt&&Xt.onHover&&Qt){
    var _hr=Xt.onHover(kt,zt,Qt);
    _dbgFs.appendFileSync("${DBG_LOG}","[HOVER] result="+JSON.stringify(_hr).substring(0,500)+"\\n\\n");
    Gt.data.result=_hr;
  }else{
    _dbgFs.appendFileSync("${DBG_LOG}","[HOVER] SKIP Xt="+!!Xt+" onHover="+!!(Xt&&Xt.onHover)+" Qt="+!!Qt+"\\n\\n");
  }
}catch(_e){
  try{require("fs").appendFileSync("${DBG_LOG}","[HOVER] ERR: "+_e.message+"\\n"+_e.stack+"\\n\\n")}catch(_e2){}
}
...
// Write patched version OVER the original
fs.writeFileSync(WORKER_FILE, patchedWorker)
...
async function main() {
    console.log('🔬 Direct Worker Patch Test v4')
    ...
    for (let i = 0; i < lines.length; i++) {
        if (lines[i].includes('this.pageStack')) {
            ...
            console.log(`🔍 Hover pageStack L${i}:${c}`)
            await aceReq('aceProject/onAsyncHover', { textDocument: { uri }, position: { line: i, character: c } })
            break
        }
    }
    ...
    await sleep(1000)
    console.log('\n' + '='.repeat(60))
    console.log('DEBUG LOG (/tmp/hover-debug.log):')
...
function cleanup() {
    ...
    try { fs.unlinkSync(path.join(ACE_DIR, 'ace-patched-index.js')) } catch (e) { }
    try { fs.unlinkSync(path.join(ACE_DIR, 'main-patched-index.js')) } catch (e) { }
    try { fs.unlinkSync(path.join(ACE_DIR, 'worker/patched-index.js')) } catch (e) { }
