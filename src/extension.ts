...
import { detectDevEco } from '../packages/ace-bridge/src/deveco-detector'
import { parseProject } from '../packages/ace-bridge/src/project-parser'
import { buildInitializationOptions } from '../packages/ace-bridge/src/module-builder'
import { launchAceServer, LaunchResult } from '../packages/ace-bridge/src/ace-launcher'
...
import { activateConductorHub, deactivateConductorHub } from '../packages/conductor-hub-vscode/src/activation'
import { SqliteClient } from '../packages/conductor-persistence/src/db/sqlite-client'
import { RequestHistoryRepository } from '../packages/conductor-hub-persistence/src/request-history'
import * as path from 'path'

let client: LanguageClient | undefined
let currentLaunch: LaunchResult | undefined
let sqliteClient: SqliteClient | undefined
...
export async function activate(context: ExtensionContext): Promise<void> {
  ...
  // ── Conductor Hub 激活（Dashboard / MCP / 状态栏 / AGC 工作流）──
  // STEP A: 初始化 SQLite 持久化存储（可失败，不阻塞核心功能）
  let storage: any = undefined
  try {
    const dataDir = context.globalStorageUri.fsPath
    sqliteClient = new SqliteClient({ dataDir, dbName: 'conductor-hub.db' })
    const historyRepo = new RequestHistoryRepository(sqliteClient)
    const dbPath = path.join(dataDir, 'db', 'conductor-hub.db')

    storage = {
      queryHistory: (page: number, pageSize: number) => historyRepo.queryHistory(page, pageSize),
      getDbPath: () => dbPath,
    }
    log('SQLite 持久化存储初始化成功 ✅')
  } catch (err: any) {
    log(`SQLite 初始化失败 (非致命 — Dashboard 仍可使用): ${err.message}`)
  }

  // STEP B: 激活 Conductor Hub 核心（状态栏 / Dashboard / MCP / AGC）
  try {
    await activateConductorHub(context, context.extensionUri, storage)
    log('Conductor Hub 激活完成 ✅')
  } catch (err: any) {
    const msg = `Conductor Hub 激活失败: ${err.message}\n${err.stack || ''}`
    log(msg)
    window.showWarningMessage(`Conductor Hub 激活异常: ${err.message}`)
  }
  ...
  // 状态栏
  const statusBar = window.createStatusBarItem(StatusBarAlignment.Right, 90)
